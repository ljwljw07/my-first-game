...
pygame.display.set_caption("벽돌 깨기 - 물리 최적화")
clock = pygame.time.Clock()
font = get_korean_font(30)
font_small = get_korean_font(16)
font_big = get_korean_font(60)

LEVELS = [{"rows": 3, "label": "레벨 1"}, {"rows": 5, "label": "레벨 2"}, {"rows": 7, "label": "레벨 3"}]
PAD_W, PAD_H = 120, 15
BALL_R_DEFAULT = 8
BLOCK_COLS = 10
BLOCK_MARGIN = 4
...
def item_selection_screen():
    ...
    items = ["1. [패시브] 상단 가속", "2. [액티브] 거대 공", "3. [액티브] 관통 모드"]
...
def main():
    ...
    level_idx, score, lives, cheat_seq, cooldown_timer, active_timer = 0, 0, 3, 0, 0, 0
    ...
    while True:
        ...
        if not launched:
            ...
            if selected_item == 1:
                ...
                bx, by = (spd if bx > 0 else -spd), (spd if by > 0 else -spd)
            ball.x += bx
            ball.y += by
            ...
            if ball.colliderect(pad) and by > 0:
                ball.bottom = pad.top
                bx, by = int((ball.centerx - pad.centerx) / (PAD_W / 2) * 7) or bx, -abs(by)
