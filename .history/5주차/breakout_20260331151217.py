...
pygame.display.set_caption("벽돌 깨기 - 무적 프레임 적용")
...
def main():
    ...
    while True:
        ...
        if not is_piercing:
            for b in blocks:
                # [수정] 충돌 체크 시 해당 벽돌의 무적 시간 확인
                if ball.colliderect(b["rect"]):
                    if now > b["last_hit_time"] + 150: # 0.15초 무적
                        hit_block = b
                    break
        
        if hit_block:
            hit_block["hp"] -= damage
            hit_block["last_hit_time"] = now # 충돌 시점 기록
            if hit_block["hp"] <= 0:
                if hit_block in blocks: blocks.remove(hit_block)
                score += 10
            
            # --- [수정된 부분] 충돌 방향 정밀 판정 ---
            # 공의 중심 x좌표가 블록의 왼쪽 끝보다 작거나, 오른쪽 끝보다 크면 옆면에 맞은 것!
            if ball.centerx < hit_block["rect"].left or ball.centerx > hit_block["rect"].right:
                bx = -bx  # 좌우 방향 반대로 튕기기
            else:
                by = -by  # 상하 방향 반대로 튕기기
