...
def main():
    ...
    while True:
        ...
        if not launched:
            ...
            for _ in range(steps):
                ...
                if ball.colliderect(pad) and by > 0:
                    ball.bottom = pad.top
                    # [유지] 요청에 따라 수직 발사 제한이 포함된 물리 로직은 수정하지 않음
                    bx = (ball.centerx - pad.centerx) / (pad.width / 2) * 7
                    by = -abs(by) # 튕겨 나가는 속도는 현재 속도 유지 (위로만 반사)
    
                    step_bx, step_by = bx, by 
                    ball_fx, ball_fy = float(ball.x), float(ball.y)
                    last_speed_up_time = now
                    break
        ...
        if not launched:
            rad = math.radians(launch_angle - 90)
            start_x, start_y = ball.centerx, ball.centery
            # 점선 형태로 궤적 표시 (약 10개의 점)
            for i in range(1, 12):
                dist = i * 25  # 점 사이의 거리
                dot_x = start_x + dist * math.cos(rad)
                dot_y = start_y + dist * math.sin(rad)
                # 화면 밖으로 나가지 않을 때만 그림
