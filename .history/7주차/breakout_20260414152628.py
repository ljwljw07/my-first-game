...
LIGHT_BLUE = (100, 200, 255) # 희귀 등급용 연한 파란색
...
BLOCK_COLS, BLOCK_MARGIN, BLOCK_TOP = 8, 4, 80 
...
def item_shop(item_id):
    common_pool = [
        ...
        {"id": "c_mini", "name": "[희귀] 미니 볼 생성", "desc": "공이 20번 튕길 때마다 새로운 미니볼 등장.", "type": "rare"},
    ...
    elif item_id == 4: # 폭발 전용 강화
        specific_pool = [
            {"id": "i4_1", "name": "[전용] 고성능 화약", "desc": "폭발 데미지가 2.0 증가합니다.", "type": "specific"},
            {"id": "i4_2", "name": "[전용] 대형 폭발", "desc": "폭발 반경이 40% 증가합니다.", "type": "specific"},
            {"id": "i4_3", "name": "[전용] 퀵 릴리즈", "desc": "폭발 쿨타임이 4초 감소합니다.", "type": "specific"}
        ]
    ...
    while True:
        ...
        for i, choice in enumerate(choices):
            ...
            if choice["type"] == "specific": color = PURPLE
            elif choice["type"] == "rare": color = LIGHT_BLUE
...
def main():
    ...
    items_info = [
        ...
        ("4. [액티브] 폭발", "[E키] 다음 충돌 시 스플래시 피해를 줍니다.")
    ]
    for i, (title, desc) in enumerate(items_info):
        card = pygame.Rect(100, 160 + i * 105, 600, 85)
        pygame.draw.rect(screen, (50, 50, 70), card, border_radius=10)
        pygame.draw.rect(screen, BLUE, card, 2, border_radius=10)
        screen.blit(font.render(title, True, WHITE), (130, 175 + i * 105))
        screen.blit(font_small.render(desc, True, (200, 200, 200)), (130, 210 + i * 105))
    pygame.display.flip()
    ...
    def register_bounce():
        ...
        if miniball_active and bounce_count % 20 == 0:
            ...
            spd = 6 + base_speed_mod
            miniballs.append({
                "rect": pygame.Rect(ball.centerx, ball.centery, 10, 10),
    ...
    while True:
        ...
        for e in pygame.event.get():
            ...
            if e.type == pygame.KEYDOWN:
                if e.key == pygame.K_SPACE and not launched:
                    safe_angle = launch_angle
                    if abs(safe_angle) < 5: safe_angle = 5 if safe_angle >= 0 else -5
                    rad = math.radians(safe_angle - 90)
                ...
                if e.key == pygame.K_e and now > cooldown_timer:
                    ...
                    elif selected_item == 4: # 폭발 활성화
                        exp_ready = True
                        cooldown_timer = now + (10000 - exp_cd_red)
        ...
        if not launched:
            ...
            if selected_item == 1:
                ...
                if abs(bx) < 0.5: bx = 0.5 if bx >= 0 else -0.5
            ...
            for _ in range(steps):
                ...
                if ball.colliderect(pad) and by > 0:
                    ...
                    if abs(bx) < 1.0: bx = 1.0 if bx >= 0 else -1.0
                    ...
                    if ball.colliderect(b["rect"]):
                        ...
                        if now > b["last_hit_time"] + 100:
                            ...
                            if exp_ready:
                                exp_ready = False
                                for eb in blocks[:]:
                                    dist = math.hypot(ball.centerx - eb["rect"].centerx, ball.centery - eb["rect"].centery)
                                    if dist < exp_range:
                                        eb["hp"] -= exp_dmg
                                        if eb["hp"] <= 0: 
                                            if eb in blocks: blocks.remove(eb); score += int(10 * current_score_mult)
            ...
            if ball.top > HEIGHT:
                if bottom_shield_active and not bottom_shield_used:
                    ...
                    bottom_shield_used = False 
                    miniballs.clear() # [꼼수 방지] 메인 볼 아웃 시 미니볼 전멸
                    if lives <= 0:
        ...
        for mb in miniballs[:]:
            ...
            if mb["rect"].left <= 0: mb["rect"].left = 0; mb["bx"] = abs(mb["bx"])
            if mb["rect"].right >= WIDTH: mb["rect"].right = WIDTH; mb["bx"] = -abs(mb["bx"])
            if mb["rect"].top <= 0: mb["rect"].top = 0; mb["by"] = abs(mb["by"])
            if mb["rect"].colliderect(pad) and mb["by"] > 0:
                mb["rect"].bottom = pad.top
                mb["bx"] = (mb["rect"].centerx - pad.centerx) / (pad.width / 2) * 6
                if abs(mb["bx"]) < 1.0: mb["bx"] = 1.0 if mb["bx"] >= 0 else -1.0
                mb["by"] = -abs(mb["by"])
            for b in blocks[:]:
        ...
        if not blocks:
            ...
            elif up_choice == "i4_1": exp_dmg += 2.0
            elif up_choice == "i4_2": exp_range *= 1.4
            elif up_choice == "i4_3": exp_cd_red += 4000
            level_idx += 1
            blocks, current_label, launched, bx, by = init_level(level_idx)
        ...
        screen.blit(font_small.render(f"장착: {['속도조절', '거대화', '관통', '폭발'][selected_item-1]}", True, (0, 255, 255)), (15, 35))
        ...
        if selected_item == 1:
            ...
            msg = f"지속: {dur}s" if (selected_item != 4 and dur > 0) else f"쿨타임: {cd}s"
            if selected_item == 4 and exp_ready: msg = "폭발 준비 완료! (E)"
            screen.blit(font_small.render(msg, True, GREEN if (dur > 0 or exp_ready) else WHITE), (15, 55))
        ...
        for mb in miniballs:
            pygame.draw.ellipse(screen, LIGHT_BLUE, mb["rect"])
            
        # 볼 색상 처리 (폭발 시 주황색)
        main_ball_color = (150,255,255) if is_piercing else (WHITE if not is_giant else YELLOW)
        if selected_item == 4 and exp_ready: main_ball_color = ORANGE
        pygame.draw.ellipse(screen, main_ball_color, ball)
